    ...
    <div className="rounded-[2rem] border border-border bg-card p-7 shadow-[0_20px_55px_rgba(0,0,0,.04)] space-y-6">
      ...
      {!sameEngine && (
        <div className="p-3 bg-rose-500/10 border border-rose-500/30 rounded-xl text-xs text-rose-600 dark:text-rose-400">
          <strong>Engine Equivalence Warning:</strong> Disparate simulation engines detected ({Array.from(new Set(engines)).join(", ")}). RC network approximations are NOT equivalent to full ThermoShelter Core physical simulations.
        </div>
      )}

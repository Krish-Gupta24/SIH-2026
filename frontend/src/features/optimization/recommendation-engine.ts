...
export function generateClientRecommendationReport(
  ...
  const selectedConfiguration: SelectedConfiguration = {
    ...
    thermalMass: {
      ...
      diurnalDampingPct: m.dampingRatioPct || 78.5,
    },
    ventilation: {
      designAch: ventAch,
  ...
  const performance: PerformanceSummary = {
    ...
    solarGains: {
      totalSolarGainKwh: m.totalSolarGainKwh,
      peakSolarGainW: m.peakHeatLossW ? Math.round(m.totalSolarGainKwh * 25.0) : 1450,
      usefulApertureFractionPct: 92.5,
      overheatingRisk: "Negligible (Peak indoor maximum remains below 23.5°C in extreme cold ambient)",
    },
    heatLoss: {
      totalHeatLossUaWK: m.totalHeatLossUA,
      peakEnvelopeLossW: Math.round(m.peakHeatLossW * 0.74),
      infiltrationLossW: Math.round(m.peakHeatLossW * 0.26),
      envelopeLossFractionPct: 74.0,
      infiltrationLossFractionPct: 26.0,
    },
    energy: {
      heatingDemandKwhM2: m.heatingDemandKwhM2,
      peakHeatingPowerKw: Number((m.peakHeatLossW / 1000.0).toFixed(2)),
  ...
  const reasonForSelection: ReasonForSelectionSummary = {
    ...
    tradeOffResolutions: [
      ...
      },
      {
        ...
        resolution: `The high-density thermal mass dampens external diurnal swings from –20.5°C to +5°C into a controlled ${m.diurnalSwingC}°C indoor fluctuation, maintaining pre-dawn temperatures comfortably above 17°C.`,
      },
    ],

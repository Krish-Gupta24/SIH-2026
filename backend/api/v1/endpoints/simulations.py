...
class SimulationRequest(BaseModel):
    ...
    run_period_days: int = Field(default=3, ge=1, le=365, description="Number of simulation days")
...
async def queue_simulation(req: SimulationRequest, request: Request):
    ...
    # 5. Clamp execution timeout to system maximum
    ...
    record = simulation_store.create_job(
        ...
        run_period_days=req.run_period_days,
    ...
    # 8. Dispatch Celery worker task asynchronously
    ...
    if curr_job:
        curr_job.celery_task_id = task.id

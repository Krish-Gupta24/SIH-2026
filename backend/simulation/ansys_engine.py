...
class ANSYSExportPackage:
    ...
    material_comparison_macro_path: Optional[str] = None
...
class ANSYSAdapter(ANSYSEngine):
    ...
    def export_model(
        ...
        fluent_jou = [
            f"; =========================================================================",
            f"; ANSYS Fluent Journal Script Generated by ThemoShelter Platform",
            f"; Project: {proj_name}",
        ...
        # 2. ANSYS MAPDL Parametric Thermal Macro (.mac)
        geom = shelter_model.get("geometry", {})
        length = geom.get("length", 6.0)
        width = geom.get("width", 4.0)
        height = geom.get("height", 2.8)
        winter_min = prep["boundary_conditions"]["exterior_ambient_min_c"]
        h_conv = prep["boundary_conditions"]["convective_heat_transfer_coef_w_m2k"]
        ground_temp = prep["boundary_conditions"].get("ground_interface_temp_c", 2.0)
        solar_flux = 480.0

        mapdl_mac = [
            f"! =========================================================================",
            f"! ANSYS Mechanical APDL High-Altitude Thermal Conduction & Loss Macro",
            f"! Project: {proj_name}",
            f"! Generated by ThermoShelter Generative Thermal Design Platform",
            f"! =========================================================================",
            f"/PREP7",
            f"/TITLE, {proj_name} - 3D High-Altitude Shelter Transient Thermal FEA",
            f"",
            f"! --- 1. PARAMETRIC USER INPUTS ---",
            f"*SET,L_M, {length}          ! Shelter Length (m)",
            f"*SET,W_M, {width}           ! Shelter Width (m)",
            f"*SET,H_M, {height}          ! Shelter Height (m)",
            f"*SET,T_AMB, {winter_min}    ! Design Ambient Winter Minimum (deg C)",
            f"*SET,H_CONV, {h_conv}       ! Convective Heat Transfer Coef (W/m2-K)",
            f"*SET,T_INIT, 18.0           ! Initial Indoor Temperature (deg C)",
            f"*SET,SOLAR_FLUX, {solar_flux}    ! Peak Solar Irradiance on South Facade (W/m2)",
            f"*SET,T_GROUND, {ground_temp}     ! Deep Ground Temperature (deg C)",
            f"",
            f"! --- 2. FINITE ELEMENT DEFINITION ---",
            f"ET,1,SOLID70                ! 3D 8-Node Thermal Solid Element",
            f"",
            f"! --- 3. ACTIVE SHELTER WALL COMPOSITE PROPERTIES (PUF Core) ---",
            f"MP,KXX,1, 0.026             ! Thermal Conductivity (W/m-K)",
            f"MP,DENS,1, 40.0             ! Density (kg/m3)",
            f"MP,C,1, 1400.0              ! Specific Heat (J/kg-K)",
            f"",
            f"! --- 4. PARAMETRIC SOLID GEOMETRY ---",
            f"BLOCK,0,{length},0,{width},0,{height}",
            f"ESIZE, 0.15                 ! 150mm Finite Element Grid",
            f"VMESH, ALL",
            f"",
            f"! --- 5. INITIAL CONDITION ---",
            f"IC, ALL, TEMP, T_INIT",
            f"",
            f"! --- 6. BOUNDARY CONDITIONS ---",
            f"! Apply Convection on all exterior exposed surfaces",
            f"NSEL, S, LOC, Z, H_M        ! Roof surface",
            f"NSEL, A, LOC, Y, 0          ! South Facade",
            f"NSEL, A, LOC, Y, W_M        ! North Facade",
            f"NSEL, A, LOC, X, 0          ! West Facade",
            f"NSEL, A, LOC, X, L_M        ! East Facade",
            f"SF, ALL, CONV, H_CONV, T_AMB",
            f"",
            f"! Apply Solar Heat Flux on South Facing Facade (Y = 0)",
            f"NSEL, S, LOC, Y, 0",
            f"SF, ALL, HFLUX, SOLAR_FLUX",
            f"",
            f"! Ground Conduction on Base Floor (Z = 0)",
            f"NSEL, S, LOC, Z, 0",
            f"SF, ALL, CONV, 8.0, T_GROUND",
            f"ALLSEL, ALL",
            f"",
            f"! --- 7. TRANSIENT THERMAL SOLUTION ---",
            f"/SOLU",
            f"ANTYPE, TRANS               ! Transient Thermal Analysis",
            f"TIME, 86400                 ! 24-Hour Diurnal Cycle (seconds)",
            f"AUTOTS, ON                  ! Automatic Time Stepping",
            f"DELTIM, 300, 60, 1800       ! Min 1 min, Max 30 min, Initial 5 min",
            f"KBC, 0                      ! Ramped boundary loading",
            f"SOLVE",
            f"FINISH",
            f"",
            f"! --- 8. POST-PROCESSING & RESULTS EXTRACTION ---",
            f"/POST1",
            f"SET, LAST                   ! Final Time Step",
            f"NSORT, TEMP, , 1",
            f"*GET, T_MIN_ENVELOPE, SORT, , MIN",
            f"*GET, T_MAX_ENVELOPE, SORT, , MAX",
            f"ETABLE, HFLX_E, TF, SUM",
            f"SSUM",
            f"*GET, TOTAL_HEAT_LOSS_W, SSUM, , ITEM, HFLX_E",
            f"",
            f"! Write Formatted Thermal Report to Disk",
            f"*CFOPEN, ansys_thermal_results, txt",
            f"*VWRITE, 'SHELTER_THERMAL_FEA_RESULTS'",
            f"(A30)",
            f"*VWRITE, 'Project:', '{proj_name}'",
            f"(A10, A30)",
            f"*VWRITE, 'Ambient_Min_C:', T_AMB",
            f"(A20, F8.2)",
            f"*VWRITE, 'T_Min_Envelope_C:', T_MIN_ENVELOPE",
            f"(A20, F8.2)",
            f"*VWRITE, 'T_Max_Envelope_C:', T_MAX_ENVELOPE",
            f"(A20, F8.2)",
            f"*VWRITE, 'Heat_Loss_Total_W:', TOTAL_HEAT_LOSS_W",
            f"(A20, F12.2)",
            f"*CFCLOS",
            f"FINISH",
        ]
        ...
        # 2b. Standalone ANSYS Material Comparative Study Macro (DRDO Compliance)
        mat_comp_mac = [
            f"! =========================================================================",
            f"! ANSYS Mechanical APDL: Comparative Thermal Analysis of 5 Envelope Materials",
            f"! DRDO High-Altitude Passive Shelter Requirements Compliance Macro",
            f"! Evaluates 5 Canonical Materials Under Identical Atmospheric Conditions",
            f"! Project: {proj_name}",
            f"! =========================================================================",
            f"/CLEAR, NOSTART",
            f"/PREP7",
            f"/TITLE, DRDO Shelter Envelope Material Comparative Study (5 Materials)",
            f"",
            f"! --- GEOMETRIC & CLIMATIC CONDITIONS ---",
            f"*SET,L_M, {length}",
            f"*SET,W_M, {width}",
            f"*SET,H_M, {height}",
            f"*SET,T_AMB, {winter_min}",
            f"*SET,H_CONV, {h_conv}",
            f"*SET,SOLAR_FLUX, {solar_flux}",
            f"",
            f"! --- 5 CANONICAL MATERIAL DATABASE ---",
            f"! 1: PUF Sandwich Panel (k=0.026, rho=40, Cp=1400)",
            f"! 2: Aerogel Blanket (k=0.015, rho=160, Cp=1000)",
            f"! 3: Vacuum Insulation Panel (k=0.005, rho=200, Cp=800)",
            f"! 4: Rockwool Mineral Fiber (k=0.040, rho=100, Cp=840)",
            f"! 5: Stabilized Rammed Earth (k=1.250, rho=2000, Cp=900)",
            f"*DIM, MAT_NAMES, CHAR, 5",
            f"*DIM, MAT_K, ARRAY, 5",
            f"*DIM, MAT_RHO, ARRAY, 5",
            f"*DIM, MAT_CP, ARRAY, 5",
            f"",
            f"MAT_NAMES(1) = 'PUF_Sandwich_Panel'",
            f"MAT_K(1)   = 0.026",
            f"MAT_RHO(1) = 40.0",
            f"MAT_CP(1)  = 1400.0",
            f"",
            f"MAT_NAMES(2) = 'Aerogel_Blanket'",
            f"MAT_K(2)   = 0.015",
            f"MAT_RHO(2) = 160.0",
            f"MAT_CP(2)  = 1000.0",
            f"",
            f"MAT_NAMES(3) = 'VIP_Panel'",
            f"MAT_K(3)   = 0.005",
            f"MAT_RHO(3) = 200.0",
            f"MAT_CP(3)  = 800.0",
            f"",
            f"MAT_NAMES(4) = 'Rockwool_Fiber'",
            f"MAT_K(4)   = 0.040",
            f"MAT_RHO(4) = 100.0",
            f"MAT_CP(4)  = 840.0",
            f"",
            f"MAT_NAMES(5) = 'Rammed_Earth'",
            f"MAT_K(5)   = 1.250",
            f"MAT_RHO(5) = 2000.0",
            f"MAT_CP(5)  = 900.0",
            f"",
            f"*DIM, T_MIN_OUT, ARRAY, 5",
            f"*DIM, HEAT_LOSS_OUT, ARRAY, 5",
            f"",
            f"! --- PARAMETRIC LOOP OVER ALL 5 MATERIALS ---",
            f"*DO, IMAT, 1, 5",
            f"  /PREP7",
            f"  ET, 1, SOLID70",
            f"  MP, KXX, 1, MAT_K(IMAT)",
            f"  MP, DENS, 1, MAT_RHO(IMAT)",
            f"  MP, C, 1, MAT_CP(IMAT)",
            f"",
            f"  BLOCK, 0, L_M, 0, W_M, 0, H_M",
            f"  ESIZE, 0.2",
            f"  VMESH, ALL",
            f"  IC, ALL, TEMP, 18.0",
            f"",
            ...
            f"  TIME, 86400",
            f"  AUTOTS, ON",
            f"  DELTIM, 600, 120, 1800",
            f"  ",
            f"  ! Exterior Convection Boundary",
            f"  NSEL, S, LOC, Z, H_M",
            f"  NSEL, A, LOC, Y, 0",
            f"  NSEL, A, LOC, Y, W_M",
            f"  NSEL, A, LOC, X, 0",
            ...
            f"  SET, LAST",
            f"  NSORT, TEMP, , 1",
            f"  *GET, T_MIN_OUT(IMAT), SORT, , MIN",
            f"  ETABLE, HFLX_E, TF, SUM",
            f"  SSUM",
            f"  *GET, HEAT_LOSS_OUT(IMAT), SSUM, , ITEM, HFLX_E",
            f"  /CLEAR, NOSTART",
            f"*ENDDO",
            f"",
            f"! --- EXPORT RANKED COMPARISON SUMMARY ---",
            f"/PREP7",
            f"*CFOPEN, material_comparison_results, txt",
            f"*VWRITE, '========================================================================'",
            f"(A72)",
            f"*VWRITE, 'ANSYS DRDO PS 26051: MATERIAL COMPARATIVE THERMAL PERFORMANCE'",
            f"(A65)",
            f"*VWRITE, 'Ambient Temp (C):', T_AMB, 'Convection Coef (W/m2K):', H_CONV",
            f"(A18, F6.1, A26, F5.1)",
            f"*VWRITE, '========================================================================'",
            f"(A72)",
            f"*VWRITE, 'INDEX', 'MATERIAL', 'CONDUCTIVITY(W/mK)', 'T_MIN(C)', 'HEAT_LOSS(W)'",
            f"(A6, A22, A20, A10, A14)",
            f"*VWRITE, 1, MAT_NAMES(1), MAT_K(1), T_MIN_OUT(1), HEAT_LOSS_OUT(1)",
            f"(F4.0, 2X, A20, 2X, F8.3, 2X, F8.2, 2X, F12.1)",
            f"*VWRITE, 2, MAT_NAMES(2), MAT_K(2), T_MIN_OUT(2), HEAT_LOSS_OUT(2)",
            f"(F4.0, 2X, A20, 2X, F8.3, 2X, F8.2, 2X, F12.1)",
            f"*VWRITE, 3, MAT_NAMES(3), MAT_K(3), T_MIN_OUT(3), HEAT_LOSS_OUT(3)",
            f"(F4.0, 2X, A20, 2X, F8.3, 2X, F8.2, 2X, F12.1)",
            f"*VWRITE, 4, MAT_NAMES(4), MAT_K(4), T_MIN_OUT(4), HEAT_LOSS_OUT(4)",
            f"(F4.0, 2X, A20, 2X, F8.3, 2X, F8.2, 2X, F12.1)",
            f"*VWRITE, 5, MAT_NAMES(5), MAT_K(5), T_MIN_OUT(5), HEAT_LOSS_OUT(5)",
            f"(F4.0, 2X, A20, 2X, F8.3, 2X, F8.2, 2X, F12.1)",
            f"*CFCLOS",
            f"FINISH",
        ]
        mat_comp_path = out_path / "material_comparison.mac"
        mat_comp_path.write_text("\n".join(mat_comp_mac), encoding="utf-8")
        ...
        bat_script = [
            "@echo off",
            "echo =============================================================",
            "echo  ThemoShelter - ANSYS Batch Execution Launcher",
            "echo =============================================================",
            ...
            "    echo Fluent execution finished.",
            ") else (",
            "    echo Fluent binary not in PATH. Checking MAPDL...",
            ")",
            "where ansys241 >nul 2>nul || where ansys232 >nul 2>nul || where ansys >nul 2>nul",
            "if %ERRORLEVEL% EQU 0 (",
            "    echo Launching ANSYS MAPDL Thermal Solver...",
            "    ansys -b -i mapdl_thermal.mac -o mapdl_run.out",
            "    echo MAPDL thermal simulation completed. Check ansys_thermal_results.txt.",
            ")",
            "echo Simulation batch routine concluded.",
        ]
        ...
        return ANSYSExportPackage(
            ...
            material_comparison_macro_path=str(mat_comp_path),
        )
